...
class LookupFreeQuantization(nn.Module):
    ...
    def loss(self, x: jnp.ndarray) -> LfqCodebookOutput:
        ...
        token_bit_expansions = jnp.bitwise_and(jnp.arange(2 ** self.num_dims)[None, :], 2 ** jnp.arange(self.num_dims)[:, None]).astype(jnp.int32)
        token_log_probs = token_bit_log_probs[..., 0] @ (1 - token_bit_expansions) + token_bit_log_probs[..., 1] @ token_bit_expansions
        ...
        return LfqCodebookOutput(tokens=tokens, z=z, z_q=z_q, token_log_probs=jnp.zeros(()), commit_loss=commit_loss)
...
class CrossAttentionLayer(nn.Module):
    ...
    @nn.compact
    def __call__(self, x: jnp.ndarray, y: jnp.ndarray, *, mask_self: jnp.ndarray | None=None, mask_cross: jnp.ndarray | None=None, train: bool=True) -> jnp.ndarray:
        d_embed = x.shape[-1]
        seq_len_q = x.shape[-2]
        seq_len_k = y.shape[-2]
        if self.causal:
        ...
        skip = x
        x = nn.LayerNorm()(x)
        x = nn.MultiHeadDotProductAttention(num_heads=self.num_heads or d_embed // 64, dropout_rate=self.dropout_rate, deterministic=not train)(x, x, x, mask=mask_self)
        x = skip + x
        skip = x
        x = nn.LayerNorm()(x)
        x = nn.MultiHeadDotProductAttention(num_heads=self.num_heads or d_embed // 64, dropout_rate=self.dropout_rate, deterministic=not train)(x, y, y, mask=mask_cross)
        x = skip + x
        skip = x
        x = nn.LayerNorm()(x)
...
def sinusoidal_pe_init(_, shape: tuple[int, int]) -> jnp.ndarray:
    ...
    return jnp.concatenate([jnp.sin(position[:, jnp.newaxis] * div_term), jnp.cos(position[:, jnp.newaxis] * div_term)], axis=-1)
...
class TokenizerEncoderDecoder(nn.Module):
    ...
    @nn.compact
    def __call__(self, y: jnp.ndarray, *, train: bool=True, state_conditioning: jnp.ndarray | None=None, mask: jnp.ndarray | None=None) -> jnp.ndarray:
...
class FsqAttentionTokenizer(nn.Module):
    ...
    def setup(self):
        ...
        self.encoder = TokenizerEncoderDecoder(num_tokens=self.num_tokens, num_cross_tokens=self.data_horizon, num_layers=self.num_layers, causal=self.causal, use_state_conditioning=self.use_state_conditioning, mlp_ratio=self.mlp_ratio)
        self.codebook = FsqCodebook(input_dim=self.embed_dim, target_codebook_size=self.target_codebook_size, codebook_type='custom')
        self.decoder = TokenizerEncoderDecoder(num_tokens=self.data_horizon, num_cross_tokens=self.num_tokens, num_layers=self.num_layers, causal=self.causal, use_state_conditioning=self.use_state_conditioning, mlp_ratio=self.mlp_ratio)
        self.proj_mean = nn.Dense(self.data_dim)
        self.out_scale = self.param('out_scale', lambda _: jnp.full((), 1.0))

    def tokenize(self, action: jnp.ndarray, *, obs: jnp.ndarray | None=None, train: bool=False) -> tuple[jnp.ndarray, jnp.ndarray]:
    ...
    def loss(self, action: jnp.ndarray, *, obs: jnp.ndarray | None=None, train: bool=True) -> tuple[jnp.ndarray, dict[str, jnp.ndarray]]:
        x = self.proj(action)
