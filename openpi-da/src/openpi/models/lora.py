...
class FeedForward(nn.Module):
    ...
    def setup(self):
        self.w_gating = self.param('gating_einsum', nn.initializers.lecun_normal(in_axis=-2, out_axis=-1, batch_axis=(0,)), (2, self.features, self.hidden_dim))
        self.w_linear = self.param('linear', nn.initializers.lecun_normal(in_axis=-2, out_axis=-1), (self.hidden_dim, self.features))
        self.w_gating_lora = None
        self.w_linear_lora = None
        if self.lora_config:
            self.w_gating_lora = (self.param('gating_einsum_lora_a', self.lora_config.init_fn, (2, self.features, self.lora_config.rank)), self.param('gating_einsum_lora_b', self.lora_config.init_fn, (2, self.lora_config.rank, self.hidden_dim)))
            self.w_linear_lora = (self.param('linear_lora_a', self.lora_config.init_fn, (self.hidden_dim, self.lora_config.rank)), self.param('linear_lora_b', self.lora_config.init_fn, (self.lora_config.rank, self.features)))

    @nn.compact
    def __call__(self, x):
        dtype = x.dtype
        ff_gate = self._dot(x, self.w_gating[0], None if self.w_gating_lora is None else (self.w_gating_lora[0][0], self.w_gating_lora[1][0]))
        gate_value = nn.gelu(ff_gate)
        ff1 = self._dot(x, self.w_gating[1], None if self.w_gating_lora is None else (self.w_gating_lora[0][1], self.w_gating_lora[1][1]))
