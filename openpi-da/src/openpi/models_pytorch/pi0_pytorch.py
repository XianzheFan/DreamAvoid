...
class PI0Pytorch(nn.Module):
    ...
    def _prepare_attention_masks_4d(self, att_2d_masks):
        ...
        return torch.where(att_2d_masks_4d, 0.0, -2.3819763e+38)

    def _preprocess_observation(self, observation, *, train=True):
        observation = _preprocessing.preprocess_observation_pytorch(observation, train=train)
        return (list(observation.images.values()), list(observation.image_masks.values()), observation.tokenized_prompt, observation.tokenized_prompt_mask, observation.state)
    ...
    def embed_suffix(self, state, noisy_actions, timestep):
        ...
        if not self.pi05:
            ...
            att_masks += [1]
        time_emb = create_sinusoidal_pos_embedding(timestep, self.action_in_proj.out_features, min_period=0.004, max_period=4.0, device=timestep.device)
    ...
    def sample_actions(self, device, observation, noise=None, num_steps=10) -> Tensor:
        ...
        _, past_key_values = self.paligemma_with_expert.forward(attention_mask=prefix_att_2d_masks_4d, position_ids=prefix_position_ids, past_key_values=None, inputs_embeds=[prefix_embs, None], use_cache=True)
        dt = -1.0 / num_steps
    ...
    def denoise_step(self, state, prefix_pad_masks, past_key_values, x_t, timestep):
        ...
        outputs_embeds, _ = self.paligemma_with_expert.forward(attention_mask=full_att_2d_masks_4d, position_ids=position_ids, past_key_values=past_key_values, inputs_embeds=[None, suffix_embs], use_cache=False, adarms_cond=[None, adarms_cond])
