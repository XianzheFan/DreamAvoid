...
class RosOperator:
    ...
    def _queue_attribute_names(self):
        queue_names = ['front_image_queue', 'left_image_queue', 'right_image_queue', 'follower_left_arm_queue', 'follower_right_arm_queue']
        if self.args.use_depth_image:
            queue_names.extend(['front_depth_image_queue', 'left_depth_image_queue', 'right_depth_image_queue'])
        if self.mode == 'collection':
            queue_names.extend(['leader_left_arm_queue', 'leader_right_arm_queue', 'follower_arm_left_pose_queue', 'follower_arm_right_pose_queue'])
        else:
            queue_names.extend(['follower_arm_left_pose_queue', 'follower_arm_right_pose_queue'])
        return queue_names
    ...
    def _format_sync_failure(self, stream_map, frame_time=None):
        ...
        for name, queue in stream_map.items():
            ...
            if frame_time is None:
                ...
                stream_states.append(f'{name}:len={len(queue)},latest={latest_stamp:.6f},delta={latest_stamp - frame_time:+.4f}s')
        details = []
    ...
    def _build_single_arm_pose(self, follower_arm_pose, follower_arm_joint_state):
        return np.array([follower_arm_pose.x, follower_arm_pose.y, follower_arm_pose.z, follower_arm_pose.roll, follower_arm_pose.pitch, follower_arm_pose.yaw, follower_arm_joint_state.position[-1]])
    ...
    def _camera_queues(self):
        ...
        if self.args.use_depth_image:
            camera_queues.extend([self.front_depth_image_queue, self.left_depth_image_queue, self.right_depth_image_queue])
        return camera_queues

    def _collection_stream_map(self):
        stream_map = {'img_front': self.front_image_queue, 'img_left': self.left_image_queue, 'img_right': self.right_image_queue, 'leader_arm_left': self.leader_left_arm_queue, 'leader_arm_right': self.leader_right_arm_queue, 'follower_arm_left': self.follower_left_arm_queue, 'follower_arm_right': self.follower_right_arm_queue, 'follower_arm_left_pose': self.follower_arm_left_pose_queue, 'follower_arm_right_pose': self.follower_arm_right_pose_queue}
        if self.args.use_depth_image:
            stream_map.update({'img_front_depth': self.front_depth_image_queue, 'img_left_depth': self.left_depth_image_queue, 'img_right_depth': self.right_depth_image_queue})
        return stream_map

    def _inference_stream_map(self):
        stream_map = {'img_front': self.front_image_queue, 'img_left': self.left_image_queue, 'img_right': self.right_image_queue, 'follower_arm_left': self.follower_left_arm_queue, 'follower_arm_right': self.follower_right_arm_queue, 'follower_arm_left_pose': self.follower_arm_left_pose_queue, 'follower_arm_right_pose': self.follower_arm_right_pose_queue}
        if self.args.use_depth_image:
            stream_map.update({'img_front_depth': self.front_depth_image_queue, 'img_left_depth': self.left_depth_image_queue, 'img_right_depth': self.right_depth_image_queue})
        return stream_map
    ...
    def get_collection_frame(self):
        ...
        return (True, (img_front, img_left, img_right, img_front_depth, img_left_depth, img_right_depth, follower_arm_left, follower_arm_right, follower_arm_left_pose, follower_arm_right_pose, leader_arm_left, leader_arm_right))
    ...
    def get_inference_frame(self):
        ...
        return (True, (img_front, img_left, img_right, img_front_depth, img_left_depth, img_right_depth, follower_arm_left, follower_arm_right, follower_arm_left_pose, follower_arm_right_pose))
    ...
    def init_ros(self):
        ...
        subscriber_specs = [(self.args.img_front_topic, Image, self._make_queue_callback('front_image_queue')), (self.args.img_left_topic, Image, self._make_queue_callback('left_image_queue')), (self.args.img_right_topic, Image, self._make_queue_callback('right_image_queue')), (self.args.follower_arm_left_topic, JointState, self._make_queue_callback('follower_left_arm_queue')), (self.args.follower_arm_right_topic, JointState, self._make_queue_callback('follower_right_arm_queue'))]
        if self.args.use_depth_image:
            subscriber_specs.extend([(self.args.img_front_depth_topic, Image, self._make_queue_callback('front_depth_image_queue')), (self.args.img_left_depth_topic, Image, self._make_queue_callback('left_depth_image_queue')), (self.args.img_right_depth_topic, Image, self._make_queue_callback('right_depth_image_queue'))])
        if self.mode == 'collection':
            subscriber_specs.extend([(self.args.leader_arm_left_topic, JointState, self._make_queue_callback('leader_left_arm_queue')), (self.args.leader_arm_right_topic, JointState, self._make_queue_callback('leader_right_arm_queue')), (self.args.follower_arm_left_pose_topic, PosCmd, self._make_queue_callback('follower_arm_left_pose_queue', True)), (self.args.follower_arm_right_pose_topic, PosCmd, self._make_queue_callback('follower_arm_right_pose_queue', True))])
        else:
            subscriber_specs.extend([(self.args.follower_arm_left_pose_topic, PosCmd, self._make_queue_callback('follower_arm_left_pose_queue', True)), (self.args.follower_arm_right_pose_topic, PosCmd, self._make_queue_callback('follower_arm_right_pose_queue', True))])
        ...
        if self.mode == 'inference':
            ...
            self.follower_arm_right_publisher = rospy.Publisher(self.args.leader_arm_right_topic, JointState, queue_size=10)
    ...
    def process(self, keyboard_handler):
        ...
        print("\x1b[33mPress SPACE to stop recording, then 's' to save or 'q' to discard\x1b[0m\n")
        while not rospy.is_shutdown():
            key = keyboard_handler.get_key()
            if key == ' ':
                print("\n\x1b[33m\nRecording paused. Press 's' to SAVE or 'q' to DISCARD: \x1b[0m", end='', flush=True)
            ...
            if not success:
                ...
                continue
            count += 1
            img_front, img_left, img_right, img_front_depth, img_left_depth, img_right_depth, follower_arm_left, follower_arm_right, follower_arm_left_pose, follower_arm_right_pose, leader_arm_left, leader_arm_right = result
            observation = {'images': {self.args.camera_names[0]: img_front, self.args.camera_names[1]: img_left, self.args.camera_names[2]: img_right}}
            if self.args.use_depth_image:
                observation['images_depth'] = {self.args.camera_names[0]: img_front_depth, self.args.camera_names[1]: img_left_depth, self.args.camera_names[2]: img_right_depth}
            ...
            eef_pose = self.build_follower_arm_pose(follower_arm_left_pose, follower_arm_right_pose, follower_arm_left, follower_arm_right)
            zero_value_error = self._format_zero_value_failure([('follower_arm_left.position', np.array(follower_arm_left.position)), ('follower_arm_right.position', np.array(follower_arm_right.position)), ('leader_arm_left.position', np.array(leader_arm_left.position)), ('leader_arm_right.position', np.array(leader_arm_right.position))])
            if zero_value_error:
            ...
            if count == 1:
                timesteps.append(dm_env.TimeStep(step_type=dm_env.StepType.FIRST, reward=None, discount=None, observation=observation))
                continue
            timesteps.append(dm_env.TimeStep(step_type=dm_env.StepType.MID, reward=None, discount=None, observation=observation))
    ...
    def follower_arm_publish_continuous(self, left_joint_positions, right_joint_positions):
        ...
        while not rospy.is_shutdown():
            ...
            if current_left_arm is None or current_right_arm is None:
                ...
                continue
            break
        left_direction = [1 if left_joint_positions[i] - current_left_arm[i] > 0 else -1 for i in range(len(left_joint_positions))]
        right_direction = [1 if right_joint_positions[i] - current_right_arm[i] > 0 else -1 for i in range(len(right_joint_positions))]
        moving = True
        step = 0
        while moving and (not rospy.is_shutdown()):
            ...
            right_diff = [abs(right_joint_positions[i] - current_right_arm[i]) for i in range(len(right_joint_positions))]
            moving = False
...
def get_ros_observation(args, ros_operator):
    ...
    while not rospy.is_shutdown():
        ...
        if not success:
            ...
            continue
        print_flag = True
        img_front, img_left, img_right, img_front_depth, img_left_depth, img_right_depth, follower_arm_left, follower_arm_right, follower_arm_left_pose, follower_arm_right_pose = frame_or_error
        return (img_front, img_left, img_right, follower_arm_left, follower_arm_right, follower_arm_left_pose, follower_arm_right_pose)
