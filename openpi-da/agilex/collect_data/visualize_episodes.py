...
import numpy as np
ARM_STATE_NAMES = ['joint0', 'joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'gripper']
EEF_STATE_NAMES = ['x', 'y', 'z', 'roll', 'pitch', 'yaw', 'gripper']
...
def load_hdf5(dataset_dir, dataset_name):
    ...
    with h5py.File(dataset_path, 'r') as root:
        ...
        image_dict = {cam_name: root[f'/observations/images/{cam_name}'][()] for cam_name in root['/observations/images'].keys()}
...
def save_video(image_dict, action, dt, video_path):
    ...
    for frame_idx in range(n_frames):
        ...
        print('frame:', frame_idx, 'left_action:', np.round(action[frame_idx][:7], 3), 'right_action:', np.round(action[frame_idx][7:], 3))
    writer.release()
...
def visualize_joints(qpos, action, plot_path):
    ...
    _plot_series([qpos, action], ['qpos', 'action'], joint_names, plot_path, 'Joint', ['orangered', 'royalblue'])
...
def main(args):
    ...
    print('hdf5 loaded!!')
    save_video(image_dict, action, dt, video_path=os.path.join(dataset_dir, dataset_name + '_video.mp4'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--frame_rate', action='store', type=int, help='frame_rate used when replaying video', default=30, required=False)
