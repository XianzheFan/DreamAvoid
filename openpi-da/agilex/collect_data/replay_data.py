...
def publish_joint_replay(joint_state_msg, timestamp, action, joint_left_publisher, joint_right_publisher):
...
def main(args):
    ...
    for index in range(len(actions)):
        ...
        if args.replay_mode == 'joint':
            publish_joint_replay(joint_state_msg, rospy.Time.now(), actions[index], joint_left_publisher, joint_right_publisher)
            print('joint replay', 'left_action:', np.round(actions[index][:7], 4), 'right_action:', np.round(actions[index][7:], 4))
        else:
            publish_eef_replay(eef_left_publisher, eef_right_publisher, eef_poses[index])
            print('eef replay', 'left_eef:', np.round(eef_poses[index][:7], 4), 'right_eef:', np.round(eef_poses[index][7:], 4))
        rate.sleep()
if __name__ == '__main__':
    ...
    parser.add_argument('--replay_mode', action='store', type=str, choices=['joint', 'eef'], default='joint', help='Replay using joint commands or end-effector pose commands.', required=False)
    parser.add_argument('--control_arm_left_topic', action='store', type=str, help='control_arm_left_topic', default='/leader/joint_left', required=False)
    parser.add_argument('--control_arm_right_topic', action='store', type=str, help='control_arm_right_topic', default='/leader/joint_right', required=False)
    parser.add_argument('--control_arm_left_pose_topic', action='store', type=str, help='control_arm_left_pose_topic', default='/follower/pos_cmd_left', required=False)
    parser.add_argument('--control_arm_right_pose_topic', action='store', type=str, help='control_arm_right_pose_topic', default='/follower/pos_cmd_right', required=False)
